...
public class CustomHttpClient extends BasicHttpClient {
   ...
   *
   * <p>e.g. You can create your own project specific http client needed for http/https/tls
   * connections or a Corporate proxy based Http client here. Sometimes you may need a simple
   * default http client e.g. HttpClients.createDefault() provided by Apache lib.
   *
   * <p>Note: If you do not override this method, the framework anyways creates a http client
   * suitable for both http/https.
   */
  @Override
   ...
   * from the test-case requests or you need to add any custom headers etc. If not overridden, The
   * framework will fall back to this default implementation to handle this.
   */
  @Override
  ...
  private void addCustomHeaders(Map<String, Object> headers) {
    ...
    LOGGER.info(
        "###Added custom headers my_key={}, x_token={} to headers", my_value, x_token_value);
  }
}
