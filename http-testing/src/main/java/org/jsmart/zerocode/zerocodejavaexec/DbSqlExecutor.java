...
public class DbSqlExecutor {
  ...
  public Map<String, List<DbResult>> fetchDbCustomersByName(String name) {
    ...
    DbResult foundResult =
        results.get(RESULTS_KEY).stream()
  ...
  private Map<String, List<DbResult>> executeSelectSql(String sqlStatement) {
    ...
    /**
     * ---------------------------------------------------------------------------------- // Your
     * code goes here. // e.g. - You can use JDBC-connection/spring JDBC template and fetch the
     * results using the above 'userName and password'
     * ----------------------------------------------------------------------------------
     */

    /**
     * Once you finished the DB execution and you will get the list of results from DB in the
     * 'results' list. Values hard coded below for the example understanding only. In reality you
     * get these results from the DB.
     */
