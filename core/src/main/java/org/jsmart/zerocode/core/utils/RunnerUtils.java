...
/** This code is used by Unit and Package runner. Hence resides at a common place. */
public class RunnerUtils {
  ...
  public static String getEnvSpecificConfigFile(String serverEnv, Class<?> testClass) {
    ...
    if (allTokens.size() >= 1 && null != getEnvPropertyValue(allTokens.get(0))) {
      ...
      final String resolvedEnvPropNameWithPrefix =
          SmartUtils.resolveToken(envPropNameWithPrefix, paramMap);

      serverEnv = suffixEnvValue(serverEnv, resolvedEnvPropNameWithPrefix);

      LOGGER.info(
          "Found env specific property: '{}={}', Hence using: '{}'",
          propertyKey,
          propertyValue,
          serverEnv);
      ...
      LOGGER.info(
          "Could not find env value for env property '{}', So using '{}'", propertyKey, serverEnv);
  ...
  public static String getFullyQualifiedUrl(
      String serviceEndPoint, String host, String port, String applicationContext) {
