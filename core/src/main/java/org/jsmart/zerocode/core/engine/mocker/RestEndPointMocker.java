...
public class RestEndPointMocker {
        ...
        .forEach(
            mockStep -> {
              ...
              String jsonBodyRequest =
                  (jsonNodeBody != null)
                      ? jsonNodeBody.toString()
                      : jsonNodeResponse.get("xmlBody").asText();

              if ("GET".equals(mockStep.getOperation())) {
                LOGGER.info("*****WireMock- Mocking the GET endpoint");
                givenThat(
                    createGetRequestBuilder(mockStep)
                ...
                givenThat(
                    createPostRequestBuilder(mockStep)
                        .willReturn(responseBuilder(mockStep, jsonBodyRequest)));
                ...
                givenThat(
                    createPutRequestBuilder(mockStep)
                        .willReturn(responseBuilder(mockStep, jsonBodyRequest)));
  ...
  public static void restartWireMock(int dynamicPort) {
    ...
    WireMock.configureFor(
        "localhost", dynamicPort); // <-- Repetition of PORT was needed, this is a wireMock bug
  }
  ...
  private static MappingBuilder createRequestBuilderWithHeaders(
      MockStep mockStep, MappingBuilder requestBuilder) {
  ...
  private static ResponseDefinitionBuilder responseBuilder(
      MockStep mockStep, String jsonBodyRequest) {
    ResponseDefinitionBuilder responseBuilder =
        aResponse().withStatus(mockStep.getResponse().get("status").asInt());
  ...
  public static int createWithVirtuosoMock(String endPointJsonApi) {
    ...
    //
    //                        if(200 != status){
    //                            logbuilder.info("Mocking virtuoso end point failed. Status: " +
    // status);
    //                            throw new RuntimeException("Mocking virtuoso end point failed.
    // Status: " + status + ". Check tunnel etc.");
    //                        }
    //                    });
    //        }
