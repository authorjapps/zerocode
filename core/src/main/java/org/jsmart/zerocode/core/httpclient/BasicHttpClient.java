...
public class BasicHttpClient {
   ...
   * org.jsmart.zerocode.core.httpclient.ssl.SslTrustHttpClient#createHttpClient() {@code See
   * examples: - org.jsmart.zerocode.core.httpclient.ssl.SslTrustHttpClient#createHttpClient() -
  ...
  /**
   * Override this method in case you want to execute the http call differently via your http
   * client. Otherwise the framework falls back to this implementation by default.
   *
  ...
  public Response execute(
    ...
    RequestBuilder requestBuilder =
        createRequestBuilder(httpUrl, methodName, headers, reqBodyAsString);
  ...
  /**
   * Once the client executes the http call, then it receives the http response. This method takes
   * care of handling that. In case you need to handle it differently you can override this method.
   *
  ...
  /**
   * Override this method in case you want to make the Charset response differently for your
   * project. Otherwise the framework falls back to this implementation by default which means- If
   * the Charset is not set by the server framework will default to Charset.defaultCharset(),
   * otherwise it will use the Charset sent by the server e.g. UAT-8 or UTF-16 or UTF-32 etc.
   *
   * <p>Note- See implementation of java.nio.charset.Charset#defaultCharset. Here the default is
   * UTF-8 if the defaultCharset is not set by the JVM, otherwise it picks the JVM provided
   * defaultCharset
   *
   * @param httpResponse
   * @return : A http response compatible with Charset received from the http server e.g. UTF-8,
   *     UTF-16 etc
   * @throws IOException
   */
  ...
  /**
   * If(optionally) query parameters was sent as a JSON in the request below, this gets available to
   * this method for processing them with the url.
   *
   * <pre>{@code
   * e.g.
   ...
   *
   * <p>In case you need to handle it differently you can override this method to change this
   * behaviour to roll your own feature.
   *
  ...
  /**
   * The framework will fall back to this default implementation to handle the headers. If you want
   * to override any headers, you can do that by overriding the amendRequestHeaders(headers) method.
   *
   * @param headers
  ...
  /**
   * Override this method only in case you want to - Add more headers to the http request or - Amend
   * or modify the headers which were supplied from the JSON test-case request step.
   *
   ...
   * Otherwise the framework falls back to this default implementation. You can override this method
   * via @UseHttpClient(YourCustomHttpClient.class)
   *
   * @param body
   * @return
   */
  ...
  /**
   * This is the usual http request builder most widely used using Apache Http Client. In case you
   * want to build or prepare the requests differently, you can override this method.
   *
   * <p>Please see the following request builder to handle file uploads. -
   * BasicHttpClient#createFileUploadRequestBuilder(java.lang.String, java.lang.String,
   * java.lang.String)
   *
   * <p>You can override this method via @UseHttpClient(YourCustomHttpClient.class)
   *
   * @param httpUrl
   * @param methodName
   ...
   * @return
   */
  public RequestBuilder createDefaultRequestBuilder(
      String httpUrl, String methodName, String reqBodyAsString) {
    RequestBuilder requestBuilder = RequestBuilder.create(methodName).setUri(httpUrl);

    if (reqBodyAsString != null) {
      HttpEntity httpEntity =
          EntityBuilder.create().setContentType(APPLICATION_JSON).setText(reqBodyAsString).build();
   ...
   * is passed in the request. In case you want to build or prepare the requests differently, you
   * can override this method via @UseHttpClient(YourCustomHttpClient.class).
   *
   * @param httpUrl
   * @param methodName
   * @param reqBodyAsString
   ...
   * @throws IOException
   */
  public RequestBuilder createFormUrlEncodedRequestBuilder(
      String httpUrl, String methodName, String reqBodyAsString) throws IOException {
    RequestBuilder requestBuilder = RequestBuilder.create(methodName).setUri(httpUrl);
    if (reqBodyAsString != null) {
  ...
  /**
   * This is the http request builder for file uploads, using Apache Http Client. In case you want
   * to build or prepare the requests differently, you can override this method.
   *
   * <p>Note- With file uploads you can send more headers too from the testcase to the server,
   * except "Content-Type" because this is reserved for "multipart/form-data" which the client sends
   * to server during the file uploads. You can also send more request-params and "boundary" from
   * the test cases if needed. The boundary defaults to an unique string of local-date-time-stamp if
   ...
   * @throws IOException
   */
  public RequestBuilder createFileUploadRequestBuilder(
      String httpUrl, String methodName, String reqBodyAsString) throws IOException {
  ...
  /**
   * This method handles the http session to be maintained between the calls. In case the session is
   * not needed or to be handled differently, then this method can be overridden to do nothing or to
   * roll your own feature.
   *
  ...
  public RequestBuilder createRequestBuilder(
      String httpUrl, String methodName, Map<String, Object> headers, String reqBodyAsString)
      throws IOException {
