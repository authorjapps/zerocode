...
public class SmartUtils {
  ...
  private ObjectMapper
      mapper; // <--- remember the static methods can not use this objectMapper. So make the methods
  // non-static if you want to use this objectMapper.
  ...
  public <T> String getJsonDocumentAsString(String fileName) throws IOException {
    String jsonAsString =
        Resources.toString(
            getClass().getClassLoader().getResource(fileName), StandardCharsets.UTF_8);
    return jsonAsString;
  }
  ...
  public static String readJsonAsString(String jsonFileName) {
    try {
      ...
      throw new RuntimeException(
          "Exception occurred while reading the JSON file - " + jsonFileName);
    }
  }
  ...
  public static List<String> getAllEndPointFiles(String packageName) {
    ...
    String[] allSimulationFiles =
        jvmClassPath.findResources(packageName, new RegExpResourceFilter(".*", ".*\\.json$"));
    if (null == allSimulationFiles || allSimulationFiles.length == 0) {
      throw new RuntimeException(
          "NothingFoundHereException: Check the ("
              + packageName
              + ") integration test repo folder(empty?). ");
    }
  ...
  public List<ScenarioSpec> getScenarioSpecListByPackage(String packageName) {
    ...
    List<ScenarioSpec> scenarioSpecList =
        ...
        .forEach(
            scenarioSpec -> {
              if (!oops.add(scenarioSpec.getScenarioName())) {
                throw new RuntimeException(
                    "Oops! Can not run with multiple Scenarios with same name. Found duplicate: "
  ...
  public static String prettyPrintJson(String jsonString) {
    ...
    try {
       ...
       */
      LOGGER.error(
          "Non-JSON content was encountered. So pretty print did not format it and returned the raw text");
      return jsonString;
    }
  }
