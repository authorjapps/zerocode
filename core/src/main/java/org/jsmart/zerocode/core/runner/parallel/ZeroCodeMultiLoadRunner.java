...
public class ZeroCodeMultiLoadRunner extends ParentRunner<TestMapping[]> {
  ...
  protected void runChild(TestMapping[] childArrayElement, RunNotifier notifier) {
    ...
    Arrays.stream(childArrayElement)
        .forEach(
            thisChild -> {
    ...
    if (hasFailed) {
      String failureMessage =
          testClass.getName() + " with load/stress test(s): " + testDescription + " have Failed";
      LOGGER.error(
          failureMessage
              + ". See target/logs -or- junit granular failure report(csv) -or- fuzzy search and filter report(html) for details");
  ...
  private String validateAndGetLoadPropertiesFile() {
    ...
    if (loadWithAnno == null) {
      throw new RuntimeException(
          "Ah! You missed to put the @LoadWith(...) on the load-generating test class >> "
              + testClass.getName());
    }
  ...
  private void validateAnnotationPresence() {
    ...
    if (testMappings.length == 0) {
      throw new RuntimeException(
          "Ah! You missed to put the @TestMapping on the load-generating test class >>"
              + testClass.getName()
              + ".");
    }
  }
  ...
  private void validateTestMethod() {
    TestMapping[] testMappings = testClass.getAnnotationsByType(TestMapping.class);
    Arrays.stream(testMappings)
        .forEach(
            methodMapping -> {
              String errMessage =
                  " was invalid, please re-check and pick the correct test method to load.";
              try {
