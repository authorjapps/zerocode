...
public class StepNotificationHandler {
  ...
  Boolean handleAssertionFailed(
    ...
    LOGGER.error(
        String.format(
            "Failed assertion during Scenario:%s, --> Step:%s, Details: %s\n",
    ...
    LOGGER.error(
        prettyFailureMessage + "(See below 'Actual Vs Expected' to learn why this step failed) \n");
    notifier.fireTestFailure(new Failure(description, new RuntimeException(prettyFailureMessage)));

    return false;
  }

  Boolean handleStepException(
      RunNotifier notifier,
      Description description,
      String scenarioName,
      String stepName,
    ...
    LOGGER.info(
        String.format(
            "Exception occurred while executing Scenario:[%s], --> Step:[%s], Details: %s",
  ...
  public <A, B, C, D, E, R> R handleAssertion(
      A var1, B var2, C var3, D var4, E var5, Notifier<A, B, C, D, E, R> notifyFunc) {
  ...
  private int maxEntryLengthOf(List<FieldAssertionMatcher> failureReportList) {
    final Integer maxLength =
        ofNullable(failureReportList).orElse(Collections.emptyList()).stream()
