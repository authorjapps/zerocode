...
public class TokenUtils {
              ...
              if (runTimeToken.startsWith(inStoreToken)) {
                if (runTimeToken.startsWith(RANDOM_NUMBER)) {
                  ...
                  if (slices.length == 2) {
                    paramaMap.put(
                        runTimeToken,
                        FixedLengthRandomGenerator.getGenerator(Integer.parseInt(slices[1])));
                  } else {
                  ...
                  int length =
                      Integer.parseInt(runTimeToken.substring(RANDOM_STRING_PREFIX.length()));
  ...
  /**
   * This method was introduced later, But Framework uses-
   * ZeroCodeJsonTestProcesorImpl#getTestCaseTokens(java.lang.String)
   */
  ...
  public static String getXmlContent(String xmlFileResource) {
    try {
      ...
      throw new RuntimeException(
          "Oops! Problem occurred while reading the XML file '"
              + xmlFileResource
              + "', details:"
              + e);
    }
