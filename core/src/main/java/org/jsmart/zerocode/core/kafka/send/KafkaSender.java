...
public class KafkaSender {
  ...
  public String send(String brokers, String topicName, String requestJson)
      ...
      switch (recordType) {
        ...
        case JSON:
          ...
          if (fileName != null) {
            ...
            for (int i = 0; i < records.size(); i++) {
              deliveryDetails =
                  sendJson(topicName, producer, records.get(i), jsonRecords.getAsync());
            }
          }
        ...
        default:
          throw new RuntimeException(
              "Unsupported recordType '" + recordType + "'. Chose RAW or JSON");
      }

    } catch (Exception e) {
      LOGGER.error("Error in sending record. Exception : " + e);
      String failedStatus =
          objectMapper.writeValueAsString(new DeliveryDetails(FAILED, e.getMessage()));
  ...
  private String sendRaw(
    ...
    LOGGER.info(
        "Record was sent to partition- {}, with offset- {} ",
        metadata.partition(),
        metadata.offset());
