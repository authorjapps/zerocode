...
/**
 * Do not enable this @JsonIgnoreProperties(ignoreUnknown = true) as this will suppress the test
 * data failure, let it spit out the exception(s) in case of a bad json/test input
 */
...
public class MockStep {
  ...
  private final JsonNode
      assertions; // <-- In case the wiremock or simulator mock throws a status code etc
  ...
  public Map<String, Object> getHeadersMap() {
    ...
    try {
      ...
      if (null != headersNode) {
        headersMap =
            (HashMap<String, Object>) objectMapper.readValue(headersNode.toString(), HashMap.class);
      }
    } catch (Exception e) {
      e.printStackTrace();
