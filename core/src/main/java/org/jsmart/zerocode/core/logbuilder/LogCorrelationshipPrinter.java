...
public class LogCorrelationshipPrinter {
  ...
  public ZeroCodeReportStep buildReportSingleStep() {
    ...
    ZeroCodeReportStepBuilder zeroCodeReportStep =
  ...
  public void buildResponseDelay() {
    responseDelay =
        durationMilliSecBetween(
            requestLogBuilder.getRequestTimeStamp(), responseLogBuilder.getResponseTimeStamp());
  }

  public static double durationMilliSecBetween(
      LocalDateTime requestTimeStamp, LocalDateTime responseTimeStamp) {

    Duration dur =
        Duration.between(requestTimeStamp, responseTimeStamp != null ? responseTimeStamp : now());
    return dur.toMillis();
  }
