...
import static org.jsmart.zerocode.core.engine.preprocessor.ZeroCodeJsonTestProcesorImpl.ASSERT_VALUE_EQUAL_TO_NUMBER;
import static org.jsmart.zerocode.core.engine.preprocessor.ZeroCodeJsonTestProcesorImpl.ASSERT_VALUE_GREATER_THAN;
import static org.jsmart.zerocode.core.engine.preprocessor.ZeroCodeJsonTestProcesorImpl.ASSERT_VALUE_LESSER_THAN;
import static org.jsmart.zerocode.core.engine.preprocessor.ZeroCodeJsonTestProcesorImpl.ASSERT_VALUE_NOT_EQUAL_TO_NUMBER;
...
public class ArraySizeAsserter implements JsonAsserter {
  ...
  public FieldAssertionMatcher actualEqualsToExpected(Object result) {
    if (result instanceof JSONArray) {
      ...
      return createNotMatchingMessage(
          path, String.format("Array of size %d", expectedSize), actualArrayValue.size());
  ...
  public FieldAssertionMatcher processRelationalExpression(JSONArray actualArrayValue) {
    if (expectedSizeExpression.startsWith(ASSERT_VALUE_GREATER_THAN)) {
      String greaterThan =
          this.expectedSizeExpression.substring(ASSERT_VALUE_GREATER_THAN.length());
      ...
      String notEqualTo =
          this.expectedSizeExpression.substring(ASSERT_VALUE_NOT_EQUAL_TO_NUMBER.length());
    ...
    return createNotMatchingMessage(
        path, String.format("Array of size %s", expectedSizeExpression), actualArrayValue.size());
  }
}
