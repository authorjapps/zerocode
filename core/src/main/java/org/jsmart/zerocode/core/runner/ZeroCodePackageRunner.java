...
public class ZeroCodePackageRunner extends ParentRunner<ScenarioSpec> {
  ...
  @Inject
  public ZeroCodePackageRunner(Class<?> testClass, SmartUtils smartUtils)
  ...
  protected List<ScenarioSpec> getChildren() {
    ...
    validateSuiteAnnotationPresent(
        rootPackageAnnotation, jsonTestCasesAnnotation, scenariosAnnotation);
              ...
              testResource -> {
                try {
                  ...
                  throw new RuntimeException(
                      "Exception while deserializing to Spec. Details: " + e);
                }
              })
  ...
  /**
   * Returns a {@link Description} for {@code child}, which can be assumed to be an element of the
   * list returned by {@link ParentRunner#getChildren()}
   *
   * @param child
   */
  @Override
  protected Description describeChild(ScenarioSpec child) {
    ...
    this.scenarioDescription =
        Description.createTestDescription(testClass, child.getScenarioName());
  ...
  public void run(RunNotifier notifier) {
    ZeroCodeTestReportListener reportListener =
        new ZeroCodeTestReportListener(smartUtils.getMapper(), getInjectedReportGenerator());
  ...
  /**
   * Runs the test corresponding to {@code child}, which can be assumed to be an element of the list
   * returned by {@link ParentRunner#getChildren()}. Subclasses are responsible for making sure that
   * relevant test events are reported through {@code notifier}
   *
   * @param child
   * @param notifier
   */
  ...
  public Injector getInjector() {
    ...
    Class<? extends BasicHttpClient> runtimeHttpClient =
        runtimeClientAnnotated != null ? runtimeClientAnnotated.value() : SslTrustHttpClient.class;

    return createInjector(
        Modules.override(new ApplicationMainModule(serverEnv))
  ...
  public void setZeroCodeMultiStepsScenarioRunner(
      ZeroCodeMultiStepsScenarioRunner zeroCodeMultiStepsScenarioRunner) {
  ...
  private ZeroCodeMultiStepsScenarioRunner getInjectedMultiStepsRunner() {
    zeroCodeMultiStepsScenarioRunner =
        getInjector().getInstance(ZeroCodeMultiStepsScenarioRunner.class);
  ...
  private void handleNoRunListenerReport(ZeroCodeTestReportListener reportListener) {
    if (CHARTS_AND_CSV.equals(getProperty(ZEROCODE_JUNIT))) {
      /**
       * Gradle does not support JUnit RunListener. Hence Zerocode gracefully handled this upon
       * request from Gradle users. But this is not limited to Gradle, anywhere you want to bypass
       * the JUnit RunListener, you can achieve this way. See README for details.
       *
       * <p>There are number of tickets opened for this, but not yet fixed. -
       * https://discuss.gradle.org/t/testrunfinished-not-run-in-junit-integration/14644 -
       * https://github.com/gradle/gradle/issues/842 - many more related tickets.
       */
      LOGGER.debug(
          "Bypassed JUnit RunListener [as configured by the build tool] to generate useful reports...");
  ...
  private List<String> readTestScenarioFiles() {
    ...
    List<JsonTestCase> jsonTestCases =
        Arrays.asList(testClass.getAnnotationsByType(JsonTestCase.class));
