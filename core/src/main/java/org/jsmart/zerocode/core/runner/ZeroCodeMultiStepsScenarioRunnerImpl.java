...
public class ZeroCodeMultiStepsScenarioRunnerImpl implements ZeroCodeMultiStepsScenarioRunner {
  ...
  @Inject private ZeroCodeJsonTestProcesor zeroCodeJsonTestProcesor;

  @Inject private ZeroCodeExternalFileProcessor extFileProcessor;

  @Inject private ZeroCodeParameterizedProcessor parameterizedProcessor;

  @Inject private ApiServiceExecutor serviceExecutor;
  ...
  @Override
  public synchronized boolean runScenario(
      ScenarioSpec scenario, RunNotifier notifier, Description description) {
    ...
    for (int scnCount = 0; scnCount < scenarioLoopTimes; scnCount++) {
      ...
      ScenarioSpec parameterizedScenario =
          parameterizedProcessor.processParameterized(scenario, scnCount);

      // ---------------------------------
      // Build Report scenario for each k
      // ---------------------------------
      reportResultBuilder =
          newInstance().loop(scnCount).scenarioName(parameterizedScenario.getScenarioName());
      ...
      for (Step thisStep : parameterizedScenario.getSteps()) {
        ...
        for (int stepCount = 0; stepCount < stepLoopTimes; stepCount++) {
           ...
           */
          final String thisStepName =
              parameterizedStep.getName() + (stepCount == 0 ? "" : stepCount);

          stepExecutionState.addStep(thisStepName);

          String resolvedRequestJson =
              zeroCodeJsonTestProcesor.resolveStringJson(
                  requestJsonAsString, scenarioExecutionState.getResolvedScenarioState());
          ...
          for (int retryCounter = 0; retryCounter < retryMaxTimes; retryCounter++) {
            try {
              ...
              serviceName =
                  zeroCodeJsonTestProcesor.resolveStringJson(
                      serviceName, scenarioExecutionState.getResolvedScenarioState());
              ...
              switch (serviceType(serviceName, operationName)) {
                case REST_CALL:
                  ...
                  executionResult =
                      serviceExecutor.executeHttpApi(
                ...
                case JAVA_CALL:
                  ...
                  executionResult =
                      serviceExecutor.executeJavaOperation(
                          serviceName, operationName, resolvedRequestJson);
                  break;
                ...
                case KAFKA_CALL:
                  if (kafkaServers == null) {
                    throw new RuntimeException(
                        ">>> 'kafka.bootstrap.servers' property can not be null for kafka operations");
                  }
                  ...
                  executionResult =
                      serviceExecutor.executeKafkaService(
                          kafkaServers, topicName, operationName, resolvedRequestJson);
                  break;
                ...
                default:
                  throw new RuntimeException(
                      "Oops! Service Type Undecided. If it is intentional, "
              ...
              // logging assertion
              // -----------------
              List<JsonAsserter> asserters =
                  zeroCodeJsonTestProcesor.createJsonAsserters(resolvedAssertionJson);
              List<FieldAssertionMatcher> failureResults =
                  zeroCodeJsonTestProcesor.assertAllAndReturnFailed(asserters, executionResult);
              ...
              boolean ignoreStepFailures =
                  scenario.getIgnoreStepFailures() == null
                      ? false
                      : scenario.getIgnoreStepFailures();
     ...
     */
    reportBuilder.printToFile(
        scenario.getScenarioName() + logCorrelationshipPrinter.getCorrelationId() + ".json");
  ...
  private void stopWireMockServer() {
    if (null != wireMockServer) {
      ...
      wireMockServer = null;
      LOGGER.info(
          "Scenario: All mockings done via WireMock server. Dependant end points executed. Stopped WireMock.");
    }
  }
