...
public class ZeroCodeReportGeneratorImpl implements ZeroCodeReportGenerator {
                  ...
                  thisScenario -> {
                    ...
                    thisScenario
                        ...
                        .forEach(
                            thisStep -> {
                              test.getModel()
                                  .setStartTime(utilDateOf(thisStep.getRequestTimeStamp()));
                              test.getModel()
                                  .setEndTime(utilDateOf(thisStep.getResponseTimeStamp()));

                              final Status testStatus =
                                  thisStep.getResult().equals(RESULT_PASS)
                                      ? Status.PASS
                                      : Status.FAIL;
                              ...
                              ExtentTest step =
                                  test.createNode(
                                      thisStep.getName(),
                                      TEST_STEP_CORRELATION_ID + " " + thisStep.getCorrelationId());
                              ...
                              if (testStatus.equals(Status.PASS)) {
                                ...
                                step.info(
                                    MarkupHelper.createCodeBlock(
                                        thisStep.getOperation() + "\t" + thisStep.getUrl()));
                                step.info(
                                    MarkupHelper.createCodeBlock(
                                        thisStep.getRequest(), CodeLanguage.JSON));
                                step.info(
                                    MarkupHelper.createCodeBlock(
                                        thisStep.getResponse(), CodeLanguage.JSON));
                                step.fail(
                                    MarkupHelper.createCodeBlock(
  ...
  public void linkToSpikeChartIfEnabled() {
    ...
    if (spikeChartReportEnabled || spikeChartFileName != null) {
      ...
      String linkCodeToTargetSpikeChartHtml =
          String.format(
              "<code>&nbsp;&nbsp;<a href='%s' style=\"color: #006; background: #ff6;\"> %s </a></code>",
  ...
  protected String optionalAuthor(String scenarioName) {
    ...
    if (authorName == null) {
      authorName =
          scenarioName.substring(scenarioName.lastIndexOf(AUTHOR_MARKER) + AUTHOR_MARKER.length());
    }
  ...
  public void generateHighChartReport() {
    ...
    if (spikeChartReportEnabled) {
      HighChartColumnHtml highChartColumnHtml =
          convertCsvRowsToHighChartData(zeroCodeCsvFlattenedRows);
      generateHighChartReport(highChartColumnHtml);
    }
  }

  private HighChartColumnHtml convertCsvRowsToHighChartData(
      List<ZeroCodeCsvReport> zeroCodeCsvReportRows) {

    HighChartColumnHtmlBuilder highChartColumnHtmlBuilder =
        HighChartColumnHtmlBuilder.newInstance()
    ...
    ZeroCodeChartKeyValueArrayBuilder dataArrayBuilder =
        ZeroCodeChartKeyValueArrayBuilder.newInstance();

    zeroCodeCsvReportRows.forEach(
        thisRow ->
            dataArrayBuilder.kv(
                ZeroCodeChartKeyValueBuilder.newInstance()
  ...
  public void generateCsvReport(List<ZeroCodeCsvReport> zeroCodeCsvReportRows) {
    ...
    try {
      writer.writeValue(
          new File(
              TARGET_FULL_REPORT_DIR + TARGET_FULL_REPORT_CSV_FILE_NAME
              // "_" +
                      ...
                      thisResult
                          ...
                          .forEach(
                              thisStep -> {
                                ...
                                csvFileBuilder.requestTimeStamp(
                                    thisStep.getRequestTimeStamp().toString());
                                csvFileBuilder.responseTimeStamp(
                                    thisStep.getResponseTimeStamp().toString());
  ...
  public List<ZeroCodeReport> readZeroCodeReportsByPath(String reportsFolder) {
    ...
    List<ZeroCodeReport> scenarioReports =
        allEndPointFiles.stream()
            .map(
                reportJsonFile -> {
                  try {
                    ...
                    throw new RuntimeException(
                        "Exception while deserializing to ZeroCodeReport. Details: " + e);
                  }
                })
  ...
  protected void validateReportsFolderAndTheFilesExists(String reportsFolder) {
    ...
    try {
      File[] files =
          new File(reportsFolder).listFiles((dir, fileName) -> fileName.endsWith(".json"));

      ofNullable(files)
          .orElseThrow(
              () -> new RuntimeException("Somehow the '" + reportsFolder + "' has got no files."));
      ...
      final String message =
          "\n----------------------------------------------------------------------------------------\n"
              + "Somehow the '"
              + reportsFolder
              + "' is not present or has no report JSON files. \n"
              + "Possible reasons- \n"
      ...
      // TODO- Can Suppress as this error as this is only related to report. It doesn't hurt or
      // affect the tests at all.
  ...
  private String createTimeStampedFileName() {
    return HIGH_CHART_HTML_FILE_NAME + LocalDateTime.now().toString().replace(":", "-") + ".html";
  }
}
