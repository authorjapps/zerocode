...
public class ZeroCodeUnitRunner extends BlockJUnit4ClassRunner {
  ...
  protected void runChild(FrameworkMethod method, RunNotifier notifier) {
    ...
    if (jsonTestCaseAnno == null) {
      jsonTestCaseAnno =
          evalScenarioToJsonTestCase(method.getMethod().getAnnotation(Scenario.class));
    }
  ...
  private Class<? extends BasicKafkaClient> getCustomKafkaClientOrDefault() {
    ...
    return kafkaClientAnnotated != null
        ? kafkaClientAnnotated.value()
        : ZerocodeCustomKafkaClient.class;
  }
  ...
  private void runLeafJsonTest(
      RunNotifier notifier, Description description, JsonTestCase jsonTestCaseAnno) {
    if (jsonTestCaseAnno != null) {
  ...
  private final void runLeafJUnitTest(
    ...
    try {
      ...
      LOGGER.info(
          "JUnit test run completed. See the results in the console or log.  passed = {}", passed);
