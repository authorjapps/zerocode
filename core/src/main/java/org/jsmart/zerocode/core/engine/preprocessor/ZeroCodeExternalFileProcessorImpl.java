 ...
 *
 * <p>Processes the Step definition and resolves any reference to the external file.
 *
 * <p>Given a Json Java map, it digs deep into the fields and finds the references to the external
 * content in the classpath and replaces the value of this key with the content from the file.
 *
 * <p>Issue-2 - Feature requested by @UmeshIghe(Github) Github Link -
 * https://github.com/authorjapps/zerocode/issues/2
 *
 * <p>Suggestion and Recommendation: Try to keep the test cases as independent possible. Do not
 * create too much nested dependencies which adds unnecessary complexity to the project. The
 * complexity could be in terms project maintenance or interpreting the test case itself. We should
 * use the IDE features which now a days makes a lot easier in terms of dealing with JSON contents
 * to keep the testing life cycle simple.
 */
@Singleton
public class ZeroCodeExternalFileProcessorImpl implements ZeroCodeExternalFileProcessor {
  ...
  /**
   * Resolves the external file content for a place holder against a key e.g. in the payload or
   * assertions.
   *
   * <p>First the logic checks if dig-deep needed to avoid unwanted recursions. If not needed, the
   * step definition is returned intact. Otherwise calls the dig deep method to perform the
   * operation.
   *
  ...
  public Step resolveExtJsonFile(Step thisStep) {
    ...
    try {
      ...
      Map<String, Object> stepMap =
          objectMapper.readValue(stepNode.toString(), new TypeReference<Map<String, Object>>() {});
  ...
  /**
   * Digs deep into the nested map and looks for external file reference,if found, replaces the
   * place holder with the file content. This is handy when the engineers wants to drive the common
   * contents from a central place.
   *
            ...
            entry -> {
              ...
              if (value instanceof Map) {
                ...
                if (value != null && value.toString().contains(JSON_PAYLOAD_FILE)) {
                  LOGGER.info(
                      "Found external JSON file place holder = {}. Replacing with content", value);
                  ...
                  if (token != null && token.startsWith(JSON_PAYLOAD_FILE)) {
                    ...
                    try {
                      Object jsonFileContent =
                          objectMapper.readTree(readJsonAsString(resourceJsonFile));
