...
public class ExecutorServiceRunner {
  ...
  public void runRunnables() {
    ...
    if (runnables == null || runnables.size() == 0) {
      throw new RuntimeException(
          "No runnable(s) was found to run. You can add one or more runnables using 'addRunnable(Runnable runnable)'");
    }
            ...
            .forEach(
                thisFunction -> {
                  for (int j = 0; j < numberOfThreads; j++) {
                    try {
                      LOGGER.info(
                          "Waiting for the next test flight to adjust the overall ramp up time, "
                              + "waiting time in the transit now = "
                              + delayBetweenTwoThreadsInMilliSecs);
                    ...
                    LOGGER.info(
                        Thread.currentThread().getName() + " Executor - *Start... Time = " + now());

                    executorService.execute(thisFunction);

                    LOGGER.info(
                        Thread.currentThread().getName() + " Executor - *Finished Time = " + now());
                  }
                });
      }
  ...
  public void runCallableFutures() {
    ...
    if (callables == null || callables.size() == 0) {
      throw new RuntimeException(
          "No callable(s) was found to run. You can add one or more callables using 'addCallable(Callable callable)'");
    }
          ...
          .forEach(
              future -> {
                for (int j = 0; j < numberOfThreads; j++) {
                  try {
                    LOGGER.info(
                        "Waiting in the transit for next test flight to adjust overall ramp up time, wait time now = "
                            + delayBetweenTwoThreadsInMilliSecs);
                    sleep(delayBetweenTwoThreadsInMilliSecs.longValue());
                  } catch (InterruptedException e) {
                    throw new RuntimeException(e);
                  ...
                  LOGGER.info(
                      Thread.currentThread().getName()
                          + " Future execution- *Finished Time = "
                          + now());
                }
              });
  ...
  public <T extends Object> Callable<Object> createCallableFuture(
      T objectToConsumer, Consumer<T> consumer) {
    return () -> {
  ...
  private void calculateAndSetDelayBetweenTwoThreadsInSecs(int rampUpPeriod) {
    if (rampUpPeriod == 0) {
      ...
      delayBetweenTwoThreadsInMilliSecs =
          (valueOf(rampUpPeriod) / valueOf(numberOfThreads)) * 1000L;
    }
  }
