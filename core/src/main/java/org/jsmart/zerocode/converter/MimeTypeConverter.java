...
public class MimeTypeConverter implements Converter {
  ...
  /**
   * Converts input JSON string (usually escaped e.g. "{\"a\": \"b\", \"active\": true}" ) to JSON
   * block. This helps in picking a value from the JSON block via Jayway Path and assert.
   *
   * <p>See also- - method jsonBlockToJson for unescaped json to json block. - method jsonNodeToJson
   * for unescaped json to json block.
   *
  ...
  /**
   * Converts JSON Block({"a": "b", "active": true}) to JSON block See also- jsonNodeToJson which is
   * identical to jsonBlockToJson.
   *
    ...
    try {
      ...
      try {
        ...
        transformer.setOutputProperty(
            "{http://xml.apache.org/xslt}indent-amount", String.valueOf(indentType));
        transformer.transform(xmlInput, xmlOutput);
        return xmlOutput.getWriter().toString();
      } catch (Throwable t) {
        e.printStackTrace();
