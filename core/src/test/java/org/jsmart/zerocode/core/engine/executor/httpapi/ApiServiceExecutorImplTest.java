...
public class ApiServiceExecutorImplTest {
  ...
  public void willExecuteARESTCallForA_Scenario() throws Exception {
    ...
    String responseString =
        jsonServiceExecutor.executeHttpApi(HOST_WITH_CONTEXT + "/home/bathroom/1", "GET", "{}");
  ...
  public void willExecuteARESTCallForA_GET_smart_json() throws Exception {
     ...
     * End-point available: http://localhost:9998/home/bathroom/1
     */
    String scenariosJsonAsString =
        SmartUtils.readJsonAsString("06_test_with_place_holders/02_REST_end_point_GET.json");
    final ScenarioSpec scenarioSpec =
        smartUtils.getMapper().readValue(scenariosJsonAsString, ScenarioSpec.class);
    ...
    final String responseString =
        jsonServiceExecutor.executeHttpApi(
            HOST_WITH_CONTEXT + serviceName, methodName, requestJson);
    assertThat(responseString, containsString("Shower"));
    JSONAssert.assertEquals(
        "{\n"
            + "    \"status\": 200,\n"
  ...
  public void willExecuteARESTCallForA_POST() throws Exception {
    String scenariosJsonAsString =
        SmartUtils.readJsonAsString("06_test_with_place_holders/03_REST_end_point_POST.json");
    final ScenarioSpec scenarioSpec =
        smartUtils.getMapper().readValue(scenariosJsonAsString, ScenarioSpec.class);
  ...
  public void willReturnRESTResult_textNodeJson() throws Exception {
    ...
    assertThat(
        responseString,
        containsString(
            "\"valid-text-node-json\"")); // <-- Mark: This is a JSON node, so held by double
    // quotes.
  ...
  public void willReturnRESTResult_nonJsonString() throws Exception {
    String scenariosJsonAsString =
        SmartUtils.readJsonAsString(
            "06_test_with_place_holders/05_REST_end_point_nonJson_response.json");
    final ScenarioSpec scenarioSpec =
        smartUtils.getMapper().readValue(scenariosJsonAsString, ScenarioSpec.class);
    ...
    assertThat(
        responseString,
        containsString(
            "non-json")); // <-- Mark: This is a non-JSON content which is simple String, hence not
    // held by double quotes.
