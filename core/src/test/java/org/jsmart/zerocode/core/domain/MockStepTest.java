...
/** See also: RestEndPointMockerTest.java - for integration tests. */
public class MockStepTest {
  ...
  public void testSerDeSer_headers() throws Exception {
    final String aMock =
        SmartUtils.readJsonAsString("wiremock/test_mock_step_request_headers.json");

    final MockStep mockStep = objectMapper.readValue(aMock, MockStep.class);

    assertThat(mockStep.getOperation(), is("GET"));
    assertThat(
        mockStep.getRequest().get("headers").toString(),
        is("{\"key\":\"key-001\",\"secret\":\"secret-001\"}"));

    HashMap<String, Object> headersMap =
        (HashMap<String, Object>)
            objectMapper.readValue(mockStep.getRequest().get("headers").toString(), HashMap.class);
  ...
  public void testSerDeSer_body() throws Exception {
    ...
    assertThat(mockStep.getOperation(), is("GET"));
    assertThat(
        mockStep.getRequest().get("body").toString(),
        is(
            "{\"name\":\"Emma\",\"age\":33,\"address\":{\"line1\":\"address line 1\",\"line2\":\"address line 2\"}}"));

    // The followings are not needed as WireMock will accept the full JSON body as it is
    HashMap<String, Object> bodyAsMap =
        (HashMap<String, Object>)
            objectMapper.readValue(mockStep.getRequest().get("body").toString(), HashMap.class);
