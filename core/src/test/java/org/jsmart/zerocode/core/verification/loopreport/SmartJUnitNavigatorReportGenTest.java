...
public class SmartJUnitNavigatorReportGenTest {
  ...
  public void willGeneratReportLoopSceneMultiStepsLoop2_once() throws Exception {
    ...
    final String reportFileUnderTest =
        reportJsonFiles.stream()
            .filter(
                thisReportFile -> thisReportFile.contains("Loop Scenario - Will Get A bath Room"))
            .findFirst()
            .get();

    JSONAssert.assertEquals(
        SmartUtils.readJsonAsString("11_reports/03_multi_scenario_expected_report.json"),
        // mapper.readValue(new File(TARGET_REPORT_DIR + "Will Get A bath Room Multi Multi.json"),
    ...
    ZeroCodeReport stepReportContentPojo =
        mapper.readValue(new File(reportFileUnderTest), ZeroCodeReport.class);
    assertThat(stepReportContentPojo.getResults().toString(), containsString(correlationId));
  }
  ...
  public void willGenerateloopReportIfRunLoop2OneFailRun_twice() throws Exception {
    ...
    List<String> reportFileNames =
        reportJsonFiles.stream()
            .filter(
                thisReportFile ->
                    thisReportFile.contains(
                        "Loop Scenario - Will Get A bath Room with_a_failing_step"))
            .collect(Collectors.toList());
