...
public class ZeroCodeMultiStepsScenarioRunner_FailedAssertionsTest {
  ...
  private void testStep(File thisFile) {
    ...
    if (thisFile.getName().contains(SCENARIO_NAME)) {
      ...
      assertThat(
          rawJsonReport.getResults().get(0).getSteps().get(0).getAssertions(),
          containsString("Assumed Payload"));
      assertThat(
          rawJsonReport.getResults().get(0).getSteps().get(0).getAssertions(),
          containsString("Assertion Errors"));
      assertThat(
          rawJsonReport.getResults().get(0).getSteps().get(0).getAssertions(),
          containsString("Assertion jsonPath"));
