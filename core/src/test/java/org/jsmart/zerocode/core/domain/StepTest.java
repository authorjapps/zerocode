...
public class StepTest {
  ...
  public void shouldDeserializeSingleStep() throws Exception {
    String jsonDocumentAsString =
        smartUtils.getJsonDocumentAsString("01_unit_test_jsons/01_test_json_single_step.json");
    ...
    assertThat(
        ...
        is("{Content-Type=application/json;charset=UTF-8, Cookie=cookie_123}"));
  ...
  public void testVerifications_section() throws Exception {
    String jsonDocumentAsString =
        smartUtils.getJsonDocumentAsString(
            "01_unit_test_jsons/00_test_json_single_step_verifications.json");
    Step stepDeserialized = mapper.readValue(jsonDocumentAsString, Step.class);
  ...
  public void testParameterized_values() throws Exception {
    String jsonDocumentAsString =
        smartUtils.getJsonDocumentAsString(
            "01_unit_test_jsons/06_test_single_step_parameterized_value.json");
    Step stepDeserialized = mapper.readValue(jsonDocumentAsString, Step.class);
  ...
  public void testParameterized_csv() throws Exception {
    ...
    String jsonDocumentAsString =
        smartUtils.getJsonDocumentAsString(
            "01_unit_test_jsons/07_test_single_step_parameterized_csv.json");
    Step stepDeserialized = mapper.readValue(jsonDocumentAsString, Step.class);
  ...
  public void testDeserExternalStepFile() throws Exception {
    String jsonDocumentAsString =
        smartUtils.getJsonDocumentAsString("01_unit_test_jsons/05_test_external_step_reuse.json");
    Step stepDeserialized = mapper.readValue(jsonDocumentAsString, Step.class);
    assertThat(stepDeserialized, notNullValue());
  ...
  public void shouldSerializeSingleStep() throws Exception {
    ...
    assertThat(
        singleStepNode.get("request").get("headers").toString(), containsString("Content-Type"));
    assertThat(
        singleStepNode.get("request").get("queryParams").toString(), containsString("param1"));
    assertThat(
        singleStepNode.get("request").get("queryParams").toString(),
        is("{\"param1\":\"value1\",\"invId\":10101}"));

    /**
     * Note: if it is direct value, then read as e.g. asInt, asBoolean etc, If it is a JSON block,
     * then read as jsonNode.toString, do not use asText() e.g. 1. singleStepNode.toString,
     * -->Correct 2. singleStepNode.get("request").get("headers").toString() -->Correct 2.1.
     * singleStepNode.get("request").get("headers").asText() -->Wrong 3.
     * singleStepNode.get("name").asText() -->Correct
     */
  }
}
