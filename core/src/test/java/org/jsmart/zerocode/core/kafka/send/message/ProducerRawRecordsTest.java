...
public class ProducerRawRecordsTest {
  ...
  public void testProduceRecord_KV() {
    ...
    JSONAssert.assertEquals(
        "{\"topic\":\"topic2\",\"key\":124,\"value\":\"Hello\"}", jsonBack, LENIENT);

    producerRecord = new ProducerRecord("topic2", 125L, "Hello");
    jsonBack = gson.toJson(producerRecord);
    JSONAssert.assertEquals(
        "{\"topic\":\"topic2\",\"key\":125,\"value\":\"Hello\"}", jsonBack, LENIENT);

    producerRecord = new ProducerRecord("topic2", "key-123", "Hello");
    jsonBack = gson.toJson(producerRecord);
    JSONAssert.assertEquals(
        "{\"topic\":\"topic2\",\"key\":\"key-123\",\"value\":\"Hello\"}", jsonBack, LENIENT);

    producerRecord = new ProducerRecord("topic2", "key-123", "{\"name\": \"Nicola\"}");
    jsonBack = gson.toJson(producerRecord);
    assertThat(producerRecord.value(), is("{\"name\": \"Nicola\"}"));
    JSONAssert.assertEquals(
        "{\"topic\":\"topic2\",\"headers\":{\"headers\":[],\"isReadOnly\":false},\"key\":\"key-123\",\"value\":\"{\\\"name\\\": \\\"Nicola\\\"}\"}",
        jsonBack,
        LENIENT);
  }
  ...
  public void test_ProducerRecords() {
    ...
    assertThat(
        producerProducerRawRecords.getRecords().get(0).key(),
        is(101.0)); // <-- convertes to double. But no harm
  ...
  public void test_ProducerRecordsNull() {
    final String json =
        "{\n" + "\"recordType\": \"RAW\"," + "\"file\": \"abc.txt\"," + "\"async\": true" + "}";
