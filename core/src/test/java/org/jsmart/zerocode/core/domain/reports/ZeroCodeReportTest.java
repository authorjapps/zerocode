...
public class ZeroCodeReportTest {
  ...
  public void willSerialize_ToJson() throws Exception {
    ...
    ZeroCodeReport javaBuilt =
        ZeroCodeExecResultIoWriteBuilder.newInstance()
            .timeStamp(LocalDateTime.now())
            .results(
                Arrays.asList(
                    ZeroCodeExecResultBuilder.newInstance()
                        .scenarioName("scenario name")
                        .loop(1)
                        .steps(
                            Arrays.asList(
                                ZeroCodeReportStepBuilder.newInstance()
                                    .loop(3)
  ...
  public void testReport_correct_scenariosAndSteps() {
    ...
    File[] files =
        new File(TARGET_REPORT_DIR).listFiles((dir, fileName) -> fileName.endsWith(".json"));

    List<File> relevantReportFiles =
        Arrays.asList(files).stream()
            .filter(
                thisFile ->
                    thisFile.getName().contains(SCENARIO_1)
                        || thisFile.getName().contains(SCENARIO_2))
