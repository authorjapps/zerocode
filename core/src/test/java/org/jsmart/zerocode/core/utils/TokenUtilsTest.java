...
public class TokenUtilsTest {
  ...
  public void testResolveSystemProperty_PROPERTY_FOUND_javaVendor() {
    ...
    String exampleInputString =
        "zerocode-tokentest: ${SYSTEM.PROPERTY:java.vendor}"; // java.vendor = Oracle
    // Corporation
  ...
  public void testResolveSystemProperty_PROPERTY_FOUND_fileEncoing() {
    ...
    String exampleInputString =
        "zerocode-tokentest: ${SYSTEM.PROPERTY:file.encoding}"; // java.vendor = Oracle
    // Corporation
    String resolvedString = resolveKnownTokens(exampleInputString);
    String resolvedToken = resolvedString.substring("zerocode-tokentest: ".length());
  ...
  public void testResolveSystemProperty_PROPERTY_NOT_FOUND() {
    ...
    String exampleInputString =
        "zerocode-tokentest: ${SYSTEM.PROPERTY:dummy_property_one}"; // Property does not
    // exist
    String resolvedString = resolveKnownTokens(exampleInputString);
  ...
  public void testFixedRandomGenerator_success() {
    IntStream.rangeClosed(1, 19)
        .forEach(
            i -> {
              assertTrue(
                  FixedLengthRandomGenerator.getGenerator(i).generateRandomNumber().length() == i);
            });
  }
