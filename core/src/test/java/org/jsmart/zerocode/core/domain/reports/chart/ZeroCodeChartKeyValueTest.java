...
public class ZeroCodeChartKeyValueTest {
  ...
  public void serialize() throws Exception {
    ...
    ZeroCodeChartKeyValue kv1 =
        ZeroCodeChartKeyValueBuilder.newInstance().key("Scenario 1: Step1").value(100D).build();
    ZeroCodeChartKeyValue kv2 =
        ZeroCodeChartKeyValueBuilder.newInstance()
            .key("Scenario 2: Step2")
            .value(500D)
            .result(ZeroCodeChartKeyValueArrayBuilder.TEST_FAILED)
            .build();
    ...
    assertThat(
        jsonString,
        containsString(
            "[{\"key\":\"Scenario 1: Step1\",\"value\":100.0,\"result\":null},{\"key\":\"Scenario 2: Step2\",\"value\":500.0,\"result\":\"FAILED\"}]"));
  }
  ...
  public void willGenertaeCommaSeparatedKV() throws Exception {
    ...
    */
    ...
    ZeroCodeChartKeyValue kv1 =
        ZeroCodeChartKeyValueBuilder.newInstance().key("Scenario 1: Step1").value(100D).build();
    ZeroCodeChartKeyValue kv2 =
        ZeroCodeChartKeyValueBuilder.newInstance().key("Scenario 2: Step2").value(500D).build();

    List<ZeroCodeChartKeyValue> kvs = Arrays.asList(kv1, kv2);

    final String dataRowsCommaSeparated =
        kvs.stream()
    ...
    assertThat(
        kvArray,
        is("[{name: 'Scenario 1: Step1', y: 100.0}, {name: 'Scenario 2: Step2', y: 500.0}]"));

    String kvArrayAgain = ZeroCodeChartKeyValueArrayBuilder.newInstance().kv(kv1).kv(kv2).build();
    assertThat(
        kvArrayAgain,
        is("[{name: 'Scenario 1: Step1', y: 100.0}, {name: 'Scenario 2: Step2', y: 500.0}]"));
  }
