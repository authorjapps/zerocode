...
public class SmartUtilsTest {
    ...
    public void testJsonToJavaFor_jsonFileName() throws Exception {
        Step stepJava = smartUtils.jsonFileToJava("unit_test_files/01_unit_test_jsons/01_test_json_single_step.json", Step.class);
        assertThat(stepJava.getLoop(), is(3));

        ScenarioSpec scenarioJava = smartUtils.jsonFileToJava("unit_test_files/01_unit_test_jsons/02_test_json_flow_single_step.json", ScenarioSpec.class);
    ...
    public void willReadAllfileNamesFrom_TestResource() throws Exception {
        List<String> allTestCaseFiles = SmartUtils.getAllEndPointFiles("unit_test_files/01_unit_test_jsons");
        assertThat(allTestCaseFiles.size(), is(14));
        assertThat(allTestCaseFiles.get(0), is("unit_test_files/01_unit_test_jsons/00_test_json_single_step_verifications.json"));
    }

    @Test
    public void willReadAllfileNames_AND_return_FlowSpecList() throws Exception {
        List<ScenarioSpec> allTestCaseFiles = smartUtils.getScenarioSpecListByPackage("unit_test_files/04_test_flow_cases");
    ...
    public void willReadAllfiles_find_DuplicatesScenarioNamenames_old_style() throws Exception {
        smartUtils.checkDuplicateScenarios("unit_test_files/04_test_flow_cases");
    }
