...
// @UseModules(ApplicationMainModule.class) //<--- Only if you dont pass any value to it's
// constructor
public class SmartUtilsTest {
  ...
  @Inject // <---- Without this inject you can not have the ObjectMapper injected inside SmartUtils.
  // Also you cant have the Object mapper as static.
  ...
  public void testJsonToJavaFor_jsonFileName() throws Exception {
    Step stepJava =
        smartUtils.jsonFileToJava("01_unit_test_jsons/01_test_json_single_step.json", Step.class);
    assertThat(stepJava.getLoop(), is(3));

    ScenarioSpec scenarioJava =
        smartUtils.jsonFileToJava(
            "01_unit_test_jsons/02_test_json_flow_single_step.json", ScenarioSpec.class);
  ...
  public void willReadAllfileNamesFrom_TestResource() throws Exception {
    ...
    assertThat(
        allTestCaseFiles.get(0),
        is("01_unit_test_jsons/00_test_json_single_step_verifications.json"));
  }

  @Test
  public void willReadAllfileNames_AND_return_FlowSpecList() throws Exception {
    List<ScenarioSpec> allTestCaseFiles =
        smartUtils.getScenarioSpecListByPackage("04_test_flow_cases");
