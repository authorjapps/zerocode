...
public class ZeroCodeMultiStepsScenarioRunnerImplRetryTest {
  ...
  public void testRetryScenarios() {
    ...
    // note that the first step in all scenarios is the call to wiremock, hence the index starts at
    // 1
    ...
    // loop: 2, retry-max: 3
    // in the first loop-iteration, the first attempt fails. The second one succeeds, ending this
    // loop-iteration
    // in the second loop-iteration, the first attempt immediately succeeds, ending the second
    // loop-iteration
    assertStepFailed(restWithRetryReport, 1);
    ...
    ZeroCodeReport failingRestWithRetryWithinLoopReport =
        getScenarioReport(SCENARIO_FAILED_RETRY_LOOP);
    // loop: 2, retry-max: 3
  ...
  private List<File> findScenarioReportFiles(String scenarioName) {
    File[] files =
        new File(TARGET_REPORT_DIR)
            .listFiles((dir, fileName) -> fileName.matches(scenarioName + "([a-f0-9-]*)\\.json$"));
    if (files == null) {
