...
/**
 * This JUnit5 extension helps to generate load declaratively for performance testing. We can simply
 * cherry-pick existing/new JUnit5 tests and run them in parallel as configured by the
 * load-properties.
 *
 * <p>This class implements BeforeEachCallback(not BeforeAllCallback), because the load generation
 ...
 *
 * <p>Visit the Wiki page and HelloWorld repo on this for precise examples.
 */
...
public class ParallelLoadExtension implements BeforeEachCallback {
  ...
  public void beforeEach(ExtensionContext extensionContext) throws Exception {
    ...
    Arrays.stream(testMappingArray)
        .forEach(
            thisMapping -> {
  ...
  private void failTest(Method testMethod, Class<?> testClass) {
    String failureMessage =
        testClass.getName() + " with load/stress test(s): " + testMethod + " have Failed";
    LOGGER.error(
        "\n"
            + failureMessage
            + ". \n\uD83D\uDC47"
